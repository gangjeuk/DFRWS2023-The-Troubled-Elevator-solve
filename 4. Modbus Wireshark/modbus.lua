...
function get_umas_function_name(code)
    ...
    -- newly added
    elseif code == 40 then code_name = "0x28 - READ_MEMORY: Read memory blocks"
    elseif code == 41 then code_name = "0x29 - WRITE_MEMORY: Write to memory blocks"
    -- 
...
Umas_Data               = ProtoField.bytes("UMAS.Umas_Data"            , "Umas_Data"            , base.SPACE )
-------------------------------------------------------

----------- UMAS Read/Write protocol ----------------
Logic_Addr               = ProtoField.uint32("UMAS.Logic_Addr"             , "Logic_Addr"            , base.HEX)
Data_size               = ProtoField.uint16("UMAS.Data_size"             , "Data_size"            , base.DEC)
umas_protocol.fields    = { Function_Code, Pairing_Key, Functions_Code , Umas_Functions_Code , Logic_Addr, Data_size, Umas_Data}
------------------------------------------------
...
function modbus1_protocol.dissector(buffer,pinfo,tree)
    ...
    then
        ...
        -------------------------------------
        -- BEGIN OF MEMORY READ/WRITE  ------
        -------------------------------------    
        if umas_code == 40 or umas_code == 41 then
            local getData = buffer(16)
            umasSubtree:add(Logic_Addr, buffer(10,4):le_uint())
            umasSubtree:add(Data_size, buffer(14,2):le_uint())
            umasSubtree:add(Umas_Data, getData)
        else
